...
ENV AWS_REGION={{environment.AWS_REGION}}
ENV S3_BUCKET_NAME={{environment.S3_BUCKET_NAME}}
ENV TOKEN_SIGNING_SECRET={{environment.TOKEN_SIGNING_SECRET}}
ENV SECRET_KEY_BASE={{environment.SECRET_KEY_BASE}}
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/built_with_phoenix ./
